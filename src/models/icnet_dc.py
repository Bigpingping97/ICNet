...
class ICNet(nn.Cell):
    ...
    def __init__(self, nclass=19, pretrained_path="", istraining=True):
        ...
        self.conv_sub1 = nn.SequentialCell(
            _ConvBNReLU(3, 32, 3, 2, norm_layer=nn.BatchNorm2d),
            _ConvBNReLU(32, 32, 3, 2, norm_layer=nn.BatchNorm2d),
            _ConvBNReLU(32, 64, 3, 2, norm_layer=nn.BatchNorm2d)
        )
        self.istraining = istraining
        self.ppm = PyramidPoolingModule()
        ...
        self.head = _ICHead(nclass, norm_layer=nn.BatchNorm2d)
...
class CascadeFeatureFusion12(nn.Cell):
    ...
    def __init__(self, low_channels, high_channels, out_channels, nclass, norm_layer, **kwargs):
